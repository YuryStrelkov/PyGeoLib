...
class Color:
    ...
    def __getitem__(self, index: int) -> int:
        ...
        return _get_channel(self._rgba, (index & 3) * 8)  # (self._rgba & (255 << (index * 8))) >> (index * 8)

    def __setitem__(self, index: int, value: int) -> None:
        # if index < 0 or index >= 4:
        #     raise IndexError(f"RGBA color got only 4 components.\n Trying to access index: {index}")
        self._rgba = _set_channel(self._rgba, int(value) & 255, (index & 3) * 8)
    ...
    @classmethod
    def color_map_quadratic(cls, map_amount: int = 3) -> List['Color']:
        colors = []
        dx = 1.0 / (map_amount - 1)
    ...
    @classmethod
    def color_map_linear(cls, map_amount: int = 3) -> List['Color']:
        colors = []
        dx = 1.0 / (map_amount - 1) if map_amount > 1 else 1
        for i in range(map_amount):
            xi = i * dx
